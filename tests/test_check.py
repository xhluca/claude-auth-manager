...
from claude_auth_manager.check import PROBE_MARKER, estimate_probe_cost, parse_probe_result
...
def test_estimate_probe_cost_uses_current_catalog_rates() -> None:
    ...
    assert estimate_probe_cost(model, input_tokens=1_000, output_tokens=100) == Decimal("0.003200")
    ...
    output = "\n".join(
        [
            _event(
                {
                    "type": "assistant",
                    "message": {"content": [{"type": "tool_use", "name": "Glob", "id": "tool-1"}]},
                }
            ),
            _event(
                {
...
def test_probe_never_consumes_the_calling_shell_input(monkeypatch) -> None:
    ...
    assert captured["command"][captured["command"].index("--name") + 1] == "cam-tool-check"
