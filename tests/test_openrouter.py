...
from claude_auth_manager.openrouter import (
    load_catalog,
    refresh_catalog,
    validate_key,
    validate_key_shape,
...
from claude_auth_manager.paths import catalog_path, credential_path
...
def test_validate_and_refresh_use_bearer_and_persist(
    ...
    thread.start()
    monkeypatch.setenv("CLAUDE_AUTH_MANAGER_API_BASE", f"http://127.0.0.1:{server.server_port}")
    try:
        validate_key(KEY)
