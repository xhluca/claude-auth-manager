...
from claude_auth_manager import update
...
@pytest.mark.parametrize(
    ("installed", "expected"),
    [
        ("9.8.7", "Updated Claude Auth Manager from 1.2.3 to 9.8.7."),
        ("1.2.3", "Claude Auth Manager is already up to date at 1.2.3."),
    ],
)
def test_update_reports_before_and_after(installed, expected, monkeypatch, capsys) -> None:
    ...
    assert "Checking for the latest Claude Auth Manager release…" in output
    assert output.rstrip().endswith(expected)


def test_update_keeps_a_source_install_that_is_ahead_of_pypi(monkeypatch, capsys) -> None:
    monkeypatch.setattr(
        update,
        "_source_install_location",
        lambda: "/work/claude-auth-manager",
    )
...
def test_update_replaces_source_install_when_pypi_is_newer(monkeypatch, capsys) -> None:
    ...
    assert "Updated Claude Auth Manager from 0.4.0 to 0.5.0." in capsys.readouterr().out
