...
from claude_auth_manager import service
...
def test_serve_command_prefers_stable_user_shim(isolated_home, monkeypatch) -> None:
    shim = isolated_home / ".local" / "bin" / "cam"
    shim.parent.mkdir(parents=True)
    shim.write_text("shim")
    monkeypatch.setattr(
        service.shutil,
        "which",
        lambda _name: "/tmp/ephemeral-build/bin/cam",
    )
...
def test_fallback_process_is_started_and_stopped_safely(isolated_home, monkeypatch) -> None:
    ...
    process_command = service._process_command
    lookups: list[int] = []

    def briefly_empty_command(process_id: int) -> str:
        lookups.append(process_id)
        return "" if len(lookups) == 1 else process_command(process_id)

    monkeypatch.setattr(service, "_process_command", briefly_empty_command)
    try:
...
def test_stopping_router_does_not_stop_an_existing_claude_agent(isolated_home) -> None:
    ...
    agent = subprocess.Popen(
        [sys.executable, "-c", "import time; time.sleep(60)", "claude", "agent"]
    )
    pid_path = router_pid_path()
    pid_path.parent.mkdir(parents=True)
    pid_path.write_text(f"{router.pid}\n")
    try:
        assert service._stop_fallback() is True
...
def test_systemd_unit_is_private_restartable_and_owned(isolated_home, monkeypatch) -> None:
    calls: list[list[str]] = []
    monkeypatch.setattr(service, "_serve_command", lambda _port: ["/opt/cam", "serve"])
    monkeypatch.setattr(
        service.shutil,
        "which",
    ...
    assert 'ExecStart="/opt/cam" "serve"' in content
    assert stat.S_IMODE(systemd_unit_path().stat().st_mode) == 0o600
    assert calls[-1][-2:] == ["restart", "claude-auth-manager.service"]
