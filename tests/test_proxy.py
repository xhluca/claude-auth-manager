...
from claude_auth_manager import proxy
from claude_auth_manager.openrouter import write_credential
from claude_auth_manager.paths import anthropic_credential_path
from claude_auth_manager.proxy import (
    LOCAL_TOKEN_HEADER,
    HybridRouterServer,
...
from claude_auth_manager.storage import atomic_write_text
...
def test_configured_routes_keep_invisible_legacy_account_alias(monkeypatch) -> None:
    ...
    monkeypatch.setattr(proxy, "account_aliases", lambda: {old: canonical})

    routes = configured_routes([model])

    assert f"cam/anthropic/{canonical}/claude-sonnet-5" in routes
    decision = resolve_route(f"cam/anthropic/{old}/claude-sonnet-5", routes)
    assert decision.credential == old
...
def test_openrouter_route_strips_cross_provider_credentials(routing_servers) -> None:
    with request(routing_servers, f"cam/openrouter/{GLM}") as response:
...
def test_openrouter_stream_is_forwarded_before_upstream_finishes(isolated_home) -> None:
    ...
    def consume() -> None:
        with request(router, f"cam/openrouter/{GLM}") as response:
...
def test_unknown_and_unfavorited_models_fail_closed(routing_servers) -> None:
    for model in ("google/gemini", "cam/openrouter/not/selected"):
...
def test_model_classification_is_explicit() -> None:
    ...
    assert classify_model(f"cam/openrouter/{GLM}", {GLM}) == ("openrouter", GLM)
    ...
    with pytest.raises(ValueError, match="blocked on the OpenRouter route"):
        classify_model("cam/openrouter/anthropic/claude-opus-5", {"anthropic/claude-opus-5"})
...
def test_text_only_model_receives_capability_notice_and_failed_image_tool_result() -> None:
    ...
    payload = {
        "model": f"cam/openrouter/{DEEPSEEK}",
