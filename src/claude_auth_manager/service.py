...
SERVICE_MARKER = "Managed by claude-auth-manager hybrid routing"
LAUNCHD_LABEL = "io.github.xhluca.claude-auth-manager"
...
def _serve_command(port: int) -> list[str]:
    ...
    executable = shutil.which("cam") or shutil.which("claude-auth-manager")
    if executable:
        return [executable, "serve", "--port", str(port)]
    return [sys.executable, "-m", "claude_auth_manager", "serve", "--port", str(port)]
...
[Unit]
Description=Claude Auth Manager hybrid model router
...
def _stop_fallback() -> bool:
    ...
    for attempt in range(6):
        ...
        try:
            ...
            return False
        # A freshly spawned process can briefly have an empty /proc cmdline
        # while exec is completing. Keep the ownership check fail-closed, but
        # allow that bounded startup race to settle.
        if attempt < 5:
            time.sleep(0.02)
    if not command:
        raise RuntimeError(f"could not verify that process {pid} belongs to the hybrid router")
    if command and not (
        ("claude-auth-manager" in command or "claude_auth_manager" in command or "cam" in command)
