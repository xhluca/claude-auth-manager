...
PYPI_JSON_URL = "https://pypi.org/pypi/claude-auth-manager/json"
...
def _published_version() -> str | None:
    ...
    index_url = os.environ.get("CLAUDE_AUTH_MANAGER_PYPI_INDEX_URL", DEFAULT_INDEX_URL)
...
def _pip_command(*, user: bool = False) -> list[str]:
    command = [
        ...
        "--upgrade",
        "--index-url",
        os.environ.get("CLAUDE_AUTH_MANAGER_PYPI_INDEX_URL", DEFAULT_INDEX_URL),
    ]
    if user:
        command.append("--user")
...
def _upgrade_command() -> list[str]:
    """Return an in-place upgrade command for the environment running cam."""
...
def _installed_version() -> str:
    ...
    candidates = [scripts / "cam", scripts / "claude-auth-manager"]
    for name in ("cam", "claude-auth-manager"):
    ...
    for executable in candidates:
        ...
        if version and not re.search(r"\s|[\x00-\x1f\x7f]", version):
            return version
    raise RuntimeError("the updated cam executable could not be verified")


def update_installed_package(previous_version: str) -> None:
    """Upgrade cam in place and report the version transition."""
    print("Checking for the latest Claude Auth Manager release…")
    ...
    if source is not None:
        ...
        if current_tuple is None or published_tuple is None:
            raise RuntimeError(
                f"this cam build is installed from source at {source}; the published version "
        ...
        if published_tuple <= current_tuple:
            ...
            print(
                f"Claude Auth Manager source build {previous_version} {relation} the latest "
    ...
    if result.returncode != 0:
        raise RuntimeError(f"Claude Auth Manager update exited with status {result.returncode}")
    installed_version = _installed_version()
    if installed_version == previous_version:
        print(f"Claude Auth Manager is already up to date at {installed_version}.")
    else:
        print(f"Updated Claude Auth Manager from {previous_version} to {installed_version}.")
