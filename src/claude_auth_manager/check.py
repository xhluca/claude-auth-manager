...
PROBE_MARKER = "CAM_TOOL_CHECK_OK"
PROBE_FILENAME = "cam-tool-probe.txt"
...
def parse_probe_result(
    ...
    diagnostics = [stderr.strip(), final_text.strip()]
    diagnostic = "\n".join(part for part in diagnostics if part)[-2000:]
...
def probe_model(model: dict[str, Any], *, timeout: int = 180) -> ToolProbeResult:
    ...
    thread.start()
    try:
        with tempfile.TemporaryDirectory(prefix="cam-tool-check-") as directory:
