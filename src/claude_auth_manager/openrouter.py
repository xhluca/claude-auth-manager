...
def api_base() -> str:
    return os.environ.get("CLAUDE_AUTH_MANAGER_API_BASE", DEFAULT_API_BASE).rstrip("/")
...
def fetch_models(key: str) -> list[dict[str, Any]]:
    # Unlike the public catalog, this endpoint applies the authenticated key's
    # provider preferences, privacy settings, and effective guardrails.
    payload = api_json("models/user", key)
...
def save_catalog(models: list[dict[str, Any]], key_id: str = "default") -> None:
    atomic_write_json(
        catalog_path("openrouter", key_id),
        {
            "version": 1,
            "provider": "openrouter",
            "credential": key_id,
            "source": f"{api_base()}/models/user",
...
def refresh_catalog(key: str | None = None, key_id: str = "default") -> list[dict[str, Any]]:
    models = fetch_models(key or read_credential())
    save_catalog(models, key_id)
    return models


def load_catalog(key_id: str = "default") -> list[dict[str, Any]]:
    path = catalog_path("openrouter", key_id)
    try:
