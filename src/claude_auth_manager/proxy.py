...
from .fallback import FallbackState, validate_links
from .google import (
    api_base as google_api_base,
)
from .google import (
    iter_openai_sse,
    openai_to_anthropic,
...
from .registry import account_aliases, read_account_token, read_key
from .settings import favorite_models, load_preferences, refresh_managed_subagents
...
def resolve_route(model: str, routes: dict[str, dict[str, Any]]) -> RouteDecision:
    """Resolve an exact configured route or a native built-in Claude model."""
    configured = routes.get(model)
    if configured is not None:
        parsed = parse_managed_model(model)
        if parsed is None:
            raise ValueError("configured model has an invalid managed route")
        provider, credential, upstream_model = parsed
        if not credential:
            raise ValueError("managed routes must name a credential")
        if (
            configured.get("provider") != provider
            or configured.get("credential") != credential
            or configured.get("id") != upstream_model
        ):
            raise ValueError("managed route does not match its allowlist entry")
        return RouteDecision(provider, credential, upstream_model, model)
    if model in {"default", "opus", "sonnet", "haiku", "fable"} or model.startswith("claude-"):
        return RouteDecision("native", None, model, model)
    raise ValueError("model has no trusted route or is not in the favorites allowlist")


def _routing_notice(model: str) -> str:
    return (
        "Claude Auth Manager routing notice: this request uses "
        f"{model} via claude-auth-manager (cam). The selected model can change "
        "between turns. When asked which model is active, use this notice instead "
        "of earlier messages or launch-time session information."
    )


def route_managed_payload(
    body: bytes, routes: dict[str, dict[str, Any]]
) -> tuple[RouteDecision, dict[str, Any], bytes]:
    try:
    ...
    decision = resolve_route(payload["model"], routes)
    payload["model"] = decision.model
    _append_system_notice(payload, _routing_notice(decision.model))
    if decision.provider == "openrouter" and decision.model.casefold().startswith(
        GEMINI_MODEL_PREFIX
    ):
...
def configured_routes(configured: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Build current routes plus invisible aliases for sessions using pre-migration IDs."""
    routes = {managed_model(model): model for model in configured}
    for old, canonical in account_aliases().items():
        for model in configured:
            if model.get("provider") != "anthropic" or model.get("credential") != canonical:
                continue
            alias = dict(model, credential=old)
            routes[managed_model(alias)] = alias
    return routes
...
def _capability_notice(model: str, vision_hint: str) -> str:
    return (
        "Claude Auth Manager capability notice: the selected model "
...
def classify_model(model: str, favorites: set[str]) -> tuple[str, str]:
    ...
    if openrouter_model is not None:
        ...
        if openrouter_model not in favorites:
            raise ValueError("OpenRouter model is not in the cam favorites allowlist")
...
class HybridRouterHandler(BaseHTTPRequestHandler):
    ...
    server_version = "ClaudeAuthManager"
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path.split("?", 1)[0].rstrip("/") == "/healthz":
            ...
            self._json_response(
                ...
                {
                    "status": "ok",
                    "mode": "manager" if self.router.routes else "hybrid",
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            if self.router.routes:
                from .fallback_transport import forward_managed

                forward_managed(self, body)
                return
            else:
    ...
    def _upstream_headers(
        ...
        *,
        ...
        if route == "openrouter":
            key = read_key(credential, provider="openrouter") if credential else read_credential()
            headers["Authorization"] = f"Bearer {key}"
            headers["X-Title"] = "Claude Auth Manager"
        elif route == "google":
            if not credential:
            ...
            headers["X-Api-Key"] = read_key(credential, provider="anthropic-api")
        elif route == "anthropic" and credential is not None:
            headers["Authorization"] = f"Bearer {read_account_token(credential)}"
    ...
    def _forward_google(
        ...
        """Call Google's OpenAI-compatible endpoint and emit Anthropic protocol."""
        connection_type, hostname, port, base_path = _target(self.router.google_upstream)
        ...
        try:
            connection.request("POST", f"{base_path}/chat/completions", body=body, headers=headers)
            ...
            self.end_headers()
            if stream:
                for chunk in iter_openai_sse(response, model, tool_metadata):
                    self._write_chunk(chunk)
            else:
                payload = json.loads(response.read())
                if not isinstance(payload, dict):
                    raise RuntimeError("Google returned an invalid response")
                translated = json.dumps(
                    openai_to_anthropic(payload, model, tool_metadata),
                    separators=(",", ":"),
                    ensure_ascii=False,
                ).encode()
                self._write_chunk(translated)
...
class HybridRouterServer(ThreadingHTTPServer):
    ...
    def __init__(
        ...
        *,
        ...
        self.fallbacks = (
            validate_links(fallbacks, set(self.routes)) if fallbacks is not None else None
        )
        self.fallback_state = fallback_state or FallbackState()
        ...
        self.google_upstream = google_upstream or google_api_base()
        # Gemini 3 requires encrypted thought metadata to be echoed when a
        # function call is replayed with its result. Claude's Messages format
        # has no equivalent field, so retain it by opaque tool-call id while
        # this loopback router is serving the conversation.
        # Keep opaque Gemini thought signatures isolated per API credential.
        # This prevents a duplicate tool-call id on one key from being replayed
        # to another provider account.
        self.google_tool_metadata: dict[str, dict[str, dict[str, Any]]] = {}
        self._google_metadata_lock = threading.Lock()
        self.model_modalities = model_modalities or {}
        self.record_status = record_status
        super().__init__(address, HybridRouterHandler)

    def google_metadata(self, credential: str) -> dict[str, dict[str, Any]]:
        with self._google_metadata_lock:
            return self.google_tool_metadata.setdefault(credential, {})
...
def run_router(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT) -> None:
    ...
    load_preferences()
    configured = favorite_models()
    if not configured:
        raise RuntimeError("no manager favorites are configured; run cam select")
    refresh_managed_subagents(configured)
    routes = configured_routes(configured)
    server = HybridRouterServer(
        (host, port),
        local_token=read_router_token(),
        routes=routes,
        fallback_state=FallbackState(persistent=True),
    )
    from .settings import refresh_fallback_picker

    refresh_fallback_picker(server.fallback_state.active)
    try:
